...
export async function signMasterContractApproval(
  ...
  const warning = approved ? 'Give FULL access to funds in (and approved to) SureVault?' : 'Revoke access to SureVault?'
...
enum Action {
  ...
  // Any external call (except to SureVault)
  CALL = 30,
}
